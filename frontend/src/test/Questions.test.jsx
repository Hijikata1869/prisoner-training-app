...
describe('Questionsコンポーネントテスト', () => {
  ...
  it('必要な情報を入力して「質問する」ボタンをクリックすると「質問を投稿しました」と画面のモーダルに表示されること', async() => {
    render(<Questions />);
    const trainingMenuButton = screen.getByRole((role, element) => role === 'button' && element.id === 'trainingMenus');
    const stepButton = screen.getByRole((role, element) => role === 'button' && element.id === 'steps');
